...
import './index.css';
import { ScrollableProps } from './type';
...
const InternalScrollable = (props: ScrollableProps, ref: ForwardedRef<any>) => {
  const { scrollbar, onScroll, direction, children } = props;
  ...
  const methods = useMemo(() => {
    return {
      ...
      scroll: (len: number) => {
        controllerRef.current?.customScroll(len);
      },
    };
  }, []);
  ...
  }, [direction, disableInteraction, methods]);
  ...
  return (
    <div className="scrollable-wrapper" ref={containerRef} onDragStart={(e) => e.preventDefault()}>
